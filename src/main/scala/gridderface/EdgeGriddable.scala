...
object EdgeGriddable {
  def drawOnGrid(content: LineContent, position: EdgePosition, grid: SimpleGrid, g2d: Graphics2D){
    ...
    val (row2, col2, tDim) = position.orientation match {
      case EdgeOrientation.Horizontal => (row, col+1, grid.rowHeight)
      case EdgeOrientation.Vertical => (row+1, col, grid.colWidth)
    }
    ...
    content.draw(g2d, x1, y1, x2, y2, tDim)
  }
}
