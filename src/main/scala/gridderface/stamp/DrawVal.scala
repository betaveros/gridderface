...
sealed abstract class DrawVal {
  ...
  def drawRound(g2d: Graphics2D, shape: Shape)
}
case class Draw(strokeVal: StrokeVal) extends DrawVal {
  ...
  def drawRound(g2d: Graphics2D, shape: Shape) {
    g2d setStroke strokeVal.roundStroke
    g2d draw shape
  }
}
case object Fill extends DrawVal {
  def draw(g2d: Graphics2D, shape: Shape) { g2d fill shape }
  def drawRound(g2d: Graphics2D, shape: Shape) { g2d fill shape }
}
