...
class SelectedPositionManager(initselected: Option[Position] = None) extends Griddable {
  ...
  def grid(prov: GridProvider, g2d: Graphics2D): Unit = {
    selected match {
      case Some(p: CellPosition) => CellGriddable.grid(cellContent, p, prov, g2d)
      case Some(p: EdgePosition) => EdgeGriddable.grid(edgeContent, p, prov, g2d)
      case Some(p: IntersectionPosition) => IntersectionGriddable.grid(intersectionContent, p, prov, g2d)
      case None => Unit
    }
  }
